...
public static class OrderingApi
{
    ...
    [AsParameters] OrderServices services)
    {
        ...
        var buyer = await services.DbContext.Buyers
            ...
            .Include(b => b.PaymentMethods.Where(pm => pm.CardTypeId == requestPaymentMethod.CardTypeId
    ...
    private static async Task<Ok<OrderSummary[]>> GetOrdersByUserAsync([AsParameters] OrderServices services)
    {
        var userId = services.IdentityService.GetUserIdentity() ??
            throw new InvalidOperationException("User identity could not be found. This endpoint requires authorization.");
